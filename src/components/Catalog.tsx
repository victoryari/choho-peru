          ...
          {filteredProducts.map((product) => {
            ...
            return (
              ...
              >
                {/* Image panel */}
                <div>
                  <div className="relative aspect-[4/3] max-h-32 bg-slate-50 dark:bg-slate-950/80 rounded-xl flex items-center justify-center p-2 mb-2.5 overflow-hidden border border-slate-100 dark:border-slate-800">
                    <ProtectedImage
                      src={product.img}
                      alt={product.name}
                    ...
                    <div className="absolute top-2 left-2 flex flex-col gap-1">
                      {product.tags && product.tags.includes("Best Seller") && (
                        <span className="bg-gradient-to-r from-amber-500 to-orange-500 text-[8.5px] font-bold text-white uppercase px-1.5 py-0.5 rounded-full shadow-xs flex items-center gap-0.5">
                          <Sparkles className="w-2.5 h-2.5 fill-white" />
                          Best Seller
                        </span>
                      )}
                      {outOfStock ? (
                        <span className="bg-red-500/90 text-white text-[8.5px] font-extrabold px-1.5 py-0.5 rounded-full uppercase tracking-wider">
                          Sin Stock
                        </span>
                      ) : lowStock ? (
                        <span className="bg-amber-500/90 text-slate-950 text-[8.5px] font-extrabold px-1.5 py-0.5 rounded-full uppercase tracking-wider">
                          Stock Bajo ({product.stock})
                        </span>
                      ) : (
                        <span className="bg-emerald-500/90 text-white text-[8.5px] font-extrabold px-1.5 py-0.5 rounded-full uppercase tracking-wider">
                          Stock: {product.stock}
                        </span>
                      )}
                    </div>
